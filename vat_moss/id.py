...
def validate(vat_id):
    ...
    if country_prefix == 'NO':
        ...
        try:
            ...
            if 'postadresse' in info:
                address = info['postadresse']
            elif 'forretningsadresse' in info:
                address = info['forretningsadresse']

            if address:
                address_one = address['adresse']
                post_code = address['postnummer']
                town = address['poststed']
    ...
    # EU countries
        ...
        address_elements = envelope.findall('./soap:Body/vat:checkVatResponse/vat:address', namespaces)
        if not address_elements:
            # Fail loudly if the XML seems to have changed
            raise WebServiceError('Unable to find <address> tag in response from VIES')
        ...
        address_one = address_parts[0]
        post_code = address_parts[1]

    return (ID_PATTERNS[country_prefix]['country_code'], vat_id, company_name, address_one, post_code)
